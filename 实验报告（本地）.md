...
- 为 Book、Author、Publisher、Transaction 等模式提供检索，支持其全部属性
- 初始版本中将一本书的多名作者以中文逗号隔开并成一个长字符串，检索时使用 split 拆开
- 检索时采用 __contain 支持片段搜索
...
- 使用表单（新建 foms.py 放了一堆表单，然后再在 views.py 里调用）
- 手机号使用正则表达式验证
